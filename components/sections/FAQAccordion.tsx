...
import { contactFaqs, type Faq } from "@/lib/faqs";
...
export function FAQAccordion({
  ...
  heading = "Things buyers ask before they brief us.",
  lede,
  tone = "cream",
}: Props) {
  return (
    <section className={`${tone === "cream" ? "bg-cream" : "bg-bone"} py-24 md:py-32`}>
      <Container width="narrow">
        <div className="grid gap-12 md:grid-cols-[1fr_1.4fr] md:gap-20">
          ...
          <ul className="flex flex-col">
            {items.map((f, i) => (
              <li key={f.q} className="border-t border-forest-deep/15 first:border-t-0">
