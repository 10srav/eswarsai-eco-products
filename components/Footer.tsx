      ...
      <div className="mt-20 grid gap-10 border-t border-cream/15 pt-14 md:grid-cols-[2fr_1fr_1fr_1fr]">
        <div>
          <Link href="/" className="flex items-center gap-3 font-semibold">
            <span className="grid h-7 w-7 place-items-center rounded-full bg-leaf">
        ...
        <div>
          <h4 className="eyebrow opacity-50">Products</h4>
          <ul className="mt-5 flex flex-col gap-2.5 text-sm">
            {productLinks.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="opacity-85 transition-all hover:translate-x-1 hover:text-sage hover:opacity-100">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          <h4 className="eyebrow opacity-50">Company</h4>
          <ul className="mt-5 flex flex-col gap-2.5 text-sm">
            {companyLinks.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="opacity-85 transition-all hover:translate-x-1 hover:text-sage hover:opacity-100">
        ...
        <div>
          <h4 className="eyebrow opacity-50">Reach</h4>
          <ul className="mt-5 flex flex-col gap-2.5 text-sm">
            <li><a href={`tel:${company.phoneE164}`} className="opacity-85 hover:text-sage hover:opacity-100">{company.phone}</a></li>
            <li><a href={`mailto:${company.email}`} className="opacity-85 hover:text-sage hover:opacity-100">hello@eswar…</a></li>
            <li><a href={`https://wa.me/${company.whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="opacity-85 hover:text-sage hover:opacity-100">WhatsApp</a></li>
            <li><a href={company.socials.googleMaps} target="_blank" rel="noopener noreferrer" className="opacity-85 hover:text-sage hover:opacity-100">{company.address.locality}, AP</a></li>
          </ul>
        </div>
      </div>
